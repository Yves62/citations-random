...
const newCitation = document.querySelector('button')
...
function generateRandomCitation() {
    let number = randomNumber(0, citations.length);
    randomCitation = citations[number];
    displayCitation(author, citation, randomCitation)
}

function displayCitation(name, content, tab) {
    name.textContent = `" ${tab[0]} "`
    content.textContent = tab[1]
}


window.addEventListener('load', generateRandomCitation);
newCitation.addEventListener('click',generateRandomCitation);
